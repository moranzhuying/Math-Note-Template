...
放开目录组织的写法：整本书不再强制先写 `\part`，可以直接按章—节—小节三层排；同时修掉这种写法下会出现的 `0.1` 式编号。

### 优化与改进

* **编号：省略 `\part` 时不再出现 `0.1` 前缀**
    * 章号 `\thechapter` 改为条件式：写了 `\part` 时仍是「部分号.章号」（2.1、2.2…），没写 `\part` 时退回全书递增的 1、2、3。
    * 节号 `\thesection` 同样加条件：`book.cls` 里节号恒定拼成「章号.节号」（equation / figure / table 都有守卫，偏偏 section 没有），因此没有章时会出现 `0.0.1`。subsection 及更深层沿用类文件的写法，会自动跟着变。
    * 附录计数器 `\theapx` 同理：没有 `\part` 时显示为「附录 1、2…」，不再是「附录 0.1」。
    * `\counterwithin{chapter}{part}` 保留，章号仍随部分重置；**写了 `\part` 的文档编号与改动前逐字一致**。

* **目录组织：`\part` 可以省**
    * `main.tex` 的正文主体既可以写「`\part{…}` + `\input{…}`」成对结构（部分—章—节），也可以只写 `\input{…}`（章—节—小节）。
    * 新笔记的骨架可由 `Tools/new_note` 按大纲自动生成，层级模式写在 md 头部，可选 `part, chapter, section`、`chapter, section, subsection`、两层、四层等。

---

## 2026-09-11 更新概览

合并 Algebra 系各笔记项目的扩展数学记号进模板符号库，使模板与全部笔记项目共用一套完整、一致的记号体系，并同步补齐 TeXStudio 的补全条目。

### 新增功能

* **符号库扩充（12 个扩展记号）**
    * 合并旧项目 `structure.sty` 中自扩展的记号：`\Comp`（连通分支）、`\Seqs`（序列）、`\V`（向量空间）、`\rg`（秩）、`\vect`（向量化）、`\alt`（交替）、`\sym`（对称）、`\BiBal`、`\Mul`、`\MultBal`、`\algmult`（代数重数）、`\geomult`（几何重数）。
    * 归类落位：11 个代数类记号并入 [模块 VI] 6.1 代数段，`\Comp` 并入 6.3 分析段；每条均标注合并来源。
    * 各笔记项目由此不再需要自带的「模块 VIII 兼容补丁段」，扩展记号统一由模板提供，消除多份符号库各自维护的问题。
    * `\algmult`、`\geomult` 在两套旧定义中拼写不一致（AlgMult/GeoMult 与 algm/geom），经统计正文使用次数均为 0，取拼写完整的一种。

### 优化与改进

* `update_cwl.py` 的输出结果与运行位置解耦：脚本读取模板自身的符号库，因此在模板或任一笔记项目中运行，生成的补全条目完全一致。
